...
let ChatList = React.createClass({
  ...
  render: function() {
    ...
    let chatItems = this.props.chatData.chats.map(function(chatItem){
        return <ChatListItem key={chatItem.id} isOnline={(chatItem.status == "online") ? true : false} isExpanded={(expandedChatID == chatItem.id) ? true : false} expandChat={expandChat} id={chatItem.id} name={chatItem.name} imageUrl={chatItem.imageUrl} text={chatItem.text} />
    });
